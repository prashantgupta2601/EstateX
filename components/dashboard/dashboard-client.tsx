...
import { SavedSearch } from '@/lib/mock-data/saved-searches';
...
interface DashboardClientProps {
  ...
  enquiriesPromise: Promise<Enquiry[]>;
  alertsPromise: Promise<PriceAlert[]>;
}
...
export default function DashboardClient({
  ...
  const savedSearches = use(searchesPromise);
  const enquiries = use(enquiriesPromise);
  const priceAlerts = use(alertsPromise);
  ...
  const stats = [
    ...
    },
    {
      ...
      value: String(priceAlerts.length),
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <Badge className="bg-amber-500/15 text-amber-500 border-none text-[8px] font-black uppercase tracking-wider shrink-0 py-0.5 rounded-full px-2">Pending</Badge>;
      case 'replied':
        return <Badge className="bg-emerald-500/15 text-emerald-500 border-none text-[8px] font-black uppercase tracking-wider shrink-0 py-0.5 rounded-full px-2">Replied</Badge>;
      default:
        return <Badge className="bg-muted text-muted-foreground border-none text-[8px] font-black uppercase tracking-wider shrink-0 py-0.5 rounded-full px-2">Closed</Badge>;
    }
  };
        ...
        <Card className="border-border/80 bg-card/45 backdrop-blur-md rounded-2xl p-5 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/enquiries" className="text-[10px] font-bold text-muted-foreground hover:text-primary transition-colors flex items-center gap-0.5">
              <span>All ({enquiries.length})</span>
              <ArrowRight className="h-3 w-3" />
            </Link>
          </div>
